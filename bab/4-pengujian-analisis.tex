...
\subsection{Pengujian pada model \textit{bert-base-bahasa-standard-case}}

Model ini adalah model yang di-\textit{pretrained} menggunakan bahasa Melayu saja dengan mengambil berbagai sumber dataset seperti media sosial, Wikipedia, sampai Wattpad \cite{Malaya}. Model ini dilatih oleh huzeinzol05 dan menurut pembuat model, model ini seharusnya dapat digunakan baik untuk tugas - tugas berbahasa Melayu maupun tugas - tugas berbahasa Indonesia dikarenakan kesamaan pada tata bahasa dan arti suatu kata.
...
Pada gambar \ref{fig:loss_bert_bahasa} terlihat bahwa model memiliki nilai \textit{validation loss} dan \textit{training loss} yang sedikit \textit{underfitting}. Namun, apabila diuji dengan menambahkan epoch, hasil pengujian tersebut menghasilkan model yang \textit{overfitting} sehingga tidak kami gunakan sebagai nilai pembanding. Berdasarkan tabel \ref{tab: loss_bahasa}, terlihat bahwa \textit{recall} lebih tinggi dibanding \textit{precision} saat mendeteksi berita hoaks, sehingga dapat disimpulkan bahwa model mengalami
